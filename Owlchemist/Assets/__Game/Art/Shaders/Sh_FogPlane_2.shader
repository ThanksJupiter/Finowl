Shader "OWL/FogPlaneK" {
	Properties{
		...
		_ScrollSpeeds("Panning Speeds", vector) = (-1, -20, 0, 0)
	}
			...
			SubShader {
				Pass {
					...
					{						
						...
						o.color = fixed4(v.color.r, v.color.g, v.color.b, v.color.a * 1 * smoothstep(1 - _RP, 1, rim));
						o.color.a = clamp(o.color.a * lerp(_P,0, (v.vertex.z * -1) / _FO), 0, 1); // apply alpha fall off based on local z distance
