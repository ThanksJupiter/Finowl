Shader "OWL/Fog Plane" {
	Properties{
		_MainTex("Particle Texture", 2D) = "white" {}
		[HideInInspector]_FogTex("Fog Texture", 2D) = "white" {}
		[HDR]_C("Tint Color", Color) = (1,1,1,1)
		_InvFade("Soft Particles Factor", Range(0.01,3.0)) = 1.5
		_RP("Rim Fade Power", Range(0.1, 3)) = 2
		_AM("Alpha Multiplier", Range(0, 5)) = 1
		_Scroll("Panning Speeds", vector) = (-1, -1, 0, 0)
			...
			SubShader {
				Pass {
					...
					v2f vert(appdata_t v)
					{
						...
						//o.color.a = clamp(o.color.a * lerp(_P,0, v.vertex.z / _FO), 0, 1);

						o.texcoord = TRANSFORM_TEX(v.texcoord,_MainTex);
						o.texcoord += _Scroll * _Time.x;
